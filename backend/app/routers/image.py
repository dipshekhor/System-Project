...
from app.schemas import ImagePredictionRequest, VerdictResponse
...
async def analyze_image(
    ...
    Args:
        ...
        req.confidence:  0.0–1.0 (reject if < 0.5)
    """
    # ── Step 1: Reject low-confidence predictions ─────────────────────────────
    # If the image model is < 50% confident, asking the user to retake
    # is better than giving a potentially wrong verdict.
    # 0.5 is a safe default — adjust based on your teammate's model performance.
...
async def analyze_food_photo(
    ...
    if not teammate_url:
        raise HTTPException(
            status_code=503,
            detail="Teammate image model URL not configured. "
                   "Set TEAMMATE_MODEL_URL environment variable, "
                   "or use POST /api/analyze-image instead."
        )
    ...
    # ── Forward image to teammate's model ─────────────────────────────────────
    # httpx is the async version of requests — we use it here because we're
    # inside an async route handler (requests would block the event loop).
    try:
